...
class Ticketer(commands.Cog):
    ...
    def __init__(self):
        ...
        default_guild = {
            ...
            "current_ticket": 0,
            "role": None,
            "message": "Welcome to your cart, Please let us know what products you want to buy, You can check out our offers in the #products channel :) Someone will be with you shortly.",
            "active": [],
            "modlog": True,
            "closed": [],
        }
    ...
    async def account(
        ...
        if await self._check_settings(ctx):
            ...
            if not found:
                ...
                await ctx.send("You already have an open cart.")
        else:
    ...
    async def finish(self, ctx):
        ...
        for ticket in active:
            if ctx.channel.id in ticket:
                ...
                await ctx.send("cart closed.")
                active.remove(ticket)
                async with self.config.guild(ctx.guild).closed() as closed:
                    closed.append(ticket[0])
                success = True
        if not success:
            await ctx.send("This is not a store channel.")
